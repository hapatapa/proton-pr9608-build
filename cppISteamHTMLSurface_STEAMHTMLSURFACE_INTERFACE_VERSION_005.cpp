...
{
    struct ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005_LoadURL_params *params = (ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005_LoadURL_params *)args;
    struct u_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005 *iface = (u_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005 *)params->u_iface;
    char *u_pchURL = steamclient_dos_to_unix_path( params->pchURL, 1 );
    /* Redirect to native browser instead of Steam's broken CEF */
    if(u_pchURL && (strncmp(u_pchURL, "http://", 7) == 0 || strncmp(u_pchURL, "https://", 8) == 0)){
        pid_t pid = fork();
        if(pid == 0){
            execlp("xdg-open", "xdg-open", u_pchURL, (char*)NULL);
            _exit(1);
        }
        WARN("ISteamHTMLSurface::LoadURL: redirected to xdg-open: %s (pid %d)\n", u_pchURL, pid);
    } else {
...
{
    struct wow64_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005_LoadURL_params *params = (wow64_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005_LoadURL_params *)args;
    struct u_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005 *iface = (u_ISteamHTMLSurface_STEAMHTMLSURFACE_INTERFACE_VERSION_005 *)params->u_iface;
    char *u_pchURL = steamclient_dos_to_unix_path( params->pchURL, 1 );
    /* Redirect to native browser instead of Steam's broken CEF */
    if(u_pchURL && (strncmp(u_pchURL, "http://", 7) == 0 || strncmp(u_pchURL, "https://", 8) == 0)){
